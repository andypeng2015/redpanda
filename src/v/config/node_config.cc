...
node_config::node_config() noexcept
  : developer_mode(
      *this,
      "developer_mode",
      "Flag to enable developer mode, which skips most of the checks performed at startup.  Enabling `developer_mode` isn't recommended for production use.",
...
",
    ...
    if (s_dupe_i != s.cend()) {
        return fmt::format("Duplicate items in seed_servers: {}", *s_dupe_i);
    }
    return std::nullopt;
      })
...
See how the `empty_seed_starts_cluster` setting works with the `seed_servers` setting to form a cluster.
      ...
      *this,
      ...
      the property is disabled
        and no limit is applied.
              ...
              The crash
              - tracking logic is reset(to zero consecutive crashes)
                by any of the following conditions
      :

      *The broker shuts down cleanly.* One hour passes since the last crash.
        * The `redpanda.yaml` broker configuration file is updated.
        * The `startup_log` file in the
          broker's `data_directory` is manually deleted.", {.visibility = visibility::user},
      5)
