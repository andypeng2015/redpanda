...
configuration::configuration()
  ...
  , storage_max_concurrent_replay(
      *this,
      "storage_max_concurrent_replay",
      "Maximum number of partitions' logs that will be replayed concurrently at startup, or flushed concurrently on shutdown. asdasdasdasdas",
      {.needs_restart = needs_restart::no,
       .example = "2048",
       .visibility = visibility::tunable},
      1024,
...
Accepted values: [`no_check`, `check_manifest_existence`, `check_manifest_and_segment_metadata`]",
  ...
  , leader_balancer_mode(
      *this,
      "leader_balancer_mode",
      "Mode of the leader balancer for optimizing movements of leadership between shards (logical CPU cores). Enabled by `enable_leader_balancer.` Accepted Values: * `random_hill_climbing`: a shard is randomly chosen and leadership is moved to it if the load on the original shard is reduced. * `greedy_balanced_shards`: leadership movement is based on a greedy heuristic of moving leaders from the most loaded shard to the least loaded shard.",
      {.needs_restart = needs_restart::no,
